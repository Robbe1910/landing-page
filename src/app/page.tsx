    ...
    <div className="min-h-screen w-full font-sans">
      ...
      <header className="h-screen flex flex-col items-center justify-center text-center px-6 bg-gradient-to-br from-black via-gray-900 to-lime-500 text-white">
        {isClient && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }}>
            <Image src="/logo.png" alt="Logo Robbe" width={180} height={180} priority />
          </motion.div>
        )}
        {isClient && (
          <motion.h1 className="text-5xl md:text-6xl font-extrabold drop-shadow-xl mt-6" initial={{ y: -50 }} animate={{ y: 0 }} transition={{ duration: 1 }}>
            Asesor Inmobiliario & Desarrollador Web
          </motion.h1>
        )}
        {isClient && (
          <motion.p className="text-lg mt-4 max-w-xl" initial={{ y: 50 }} animate={{ y: 0 }} transition={{ duration: 1, delay: 0.3 }}>
            Encuentra la propiedad ideal y digitaliza tu negocio fácilmente
          </motion.p>
        )}
        {isClient && (
          <motion.a href={googleCalendarLink} target="_blank" rel="noopener noreferrer"
      ...
      <section className="py-16 px-6 bg-black text-white text-center">
        ...
        <p className="mt-4 text-lg max-w-2xl mx-auto">
          Soy Roberto Berrendo Eguino (Robbe), desarrollador web especializado en React, Angular y Node.js. Actualmente también soy asesor inmobiliario, combinando tecnología con bienes raíces para mejorar tu experiencia.
        </p>
        <a href="https://www.linkedin.com/in/roberto-berrendo-eguino-475b36171/" target="_blank"
      ...
      <section className="py-16 px-6 bg-white text-center">
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <motion.div className="p-6 bg-gray-50 rounded-lg shadow-md hover:shadow-xl border-l-4 border-l-lime-500"
            whileHover={{ scale: 1.05 }}>
      ...
      <section className="py-16 px-6 bg-black text-white text-center">
        ...
        <motion.a href={whatsappLink} target="_blank" className="mt-6 inline-block px-6 py-3 bg-lime-500 text-black font-bold rounded-full hover:bg-lime-400 transition duration-300"
          whileHover={{ scale: 1.05 }}>
          WhatsApp
        </motion.a>
      </section>
